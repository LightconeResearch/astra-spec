...
- **Schema review.** Read the [specification](specification.md) and the [schema reference](elements/index.md) with an adversarial eye. Ambiguity, missing constraints, and reserved-name collisions are easier to fix now than after the format has consumers.
...
4. **Update the docs alongside the schema.** Changes to `src/astra/schema/*.yaml` should also touch [`docs/specification.md`](specification.md) and the relevant tables. The auto-generated [schema reference](elements/index.md) is regenerated by `just gen-doc`.
